...
CSR_Pagada <- read_csv("C:/Users/VINROL/Desktop/Clases III PAC 2025/Seminario de Investigación/Datas para los triangulos p-p/Modelos Bayesianos/Pérdidas Pagadas/CSR_Pagada_gráficos.csv")
...
# CA - CCL Incurridas
predicted_percentiles <- CSR_Pagada[1:50,]$Outcome_Percentile
...
# PA - CCL Incurridas
predicted_percentiles <- CSR_Pagada[51:100,]$Outcome_Percentile
n <- length(predicted_percentiles)
predicted_sorted <- sort(predicted_percentiles/100)
expected_ecdf <- (1:n) / n
ks_d_manual <- max(abs(predicted_sorted - expected_ecdf)) * 100
...
# WC - CCL Incurridas
predicted_percentiles <- CSR_Pagada[101:150,]$Outcome_Percentile
n <- length(predicted_percentiles)
predicted_sorted <- sort(predicted_percentiles/100)
expected_ecdf <- (1:n) / n
ks_d_manual <- max(abs(predicted_sorted - expected_ecdf)) * 100
...
# OL - CCL Incurridas
predicted_percentiles <- CSR_Pagada[151:200,]$Outcome_Percentile
n <- length(predicted_percentiles)
predicted_sorted <- sort(predicted_percentiles/100)
expected_ecdf <- (1:n) / n
ks_d_manual <- max(abs(predicted_sorted - expected_ecdf)) * 100
...
predicted_percentiles <- CSR_Pagada$Outcome_Percentile
...
cat("  CA: n =", nrow(CSR_Pagada[1:50,]), "\n")
cat("  PA: n =", nrow(CSR_Pagada[51:100,]), "\n")
cat("  WC: n =", nrow(CSR_Pagada[101:150,]), "\n")
cat("  OL: n =", nrow(CSR_Pagada[151:200,]), "\n")
cat("Combinado: n =", nrow(CSR_Pagada), "\n")
